...
use nntp_proxy::cache::{ArticleAvailability, ArticleCache, ArticleEntry, BackendStatus, UnifiedCache};
...
async fn test_unified_cache_memory_upsert_and_get() {
    ...
    cache.upsert(msg_id.clone(), buffer.clone(), backend_id).await;
...
async fn test_unified_cache_memory_record_missing() {
    ...
    cache.record_backend_missing(msg_id.clone(), backend_id).await;
...
async fn test_unified_cache_weighted_size() {
    ...
    cache.upsert(msg_id.clone(), buffer, BackendId::from_index(0)).await;
...
fn test_availability_from_bits_empty() {
    ...
    for i in 0..8 {
        assert_eq!(avail.status(BackendId::from_index(i)), BackendStatus::Unknown);
    }
}
...
fn test_availability_from_bits_some_missing() {
    ...
    assert!(avail.has_availability_info());
    assert_eq!(avail.status(BackendId::from_index(0)), BackendStatus::Missing);
    assert_eq!(avail.status(BackendId::from_index(1)), BackendStatus::HasArticle);
    assert_eq!(avail.status(BackendId::from_index(2)), BackendStatus::Unknown);
}
...
fn test_availability_from_bits_all_missing() {
    ...
    for i in 0..8 {
        assert_eq!(avail.status(BackendId::from_index(i)), BackendStatus::Missing);
...
fn test_availability_from_bits_roundtrip() {
    ...
    assert_eq!(reconstructed.status(BackendId::from_index(0)), original.status(BackendId::from_index(0)));
    assert_eq!(reconstructed.status(BackendId::from_index(1)), original.status(BackendId::from_index(1)));
    assert_eq!(reconstructed.status(BackendId::from_index(2)), original.status(BackendId::from_index(2)));
    assert_eq!(reconstructed.status(BackendId::from_index(3)), original.status(BackendId::from_index(3)));
}
...
fn test_disk_cache_config_defaults() {
    ...
    assert_eq!(config.path, std::path::PathBuf::from("/var/cache/nntp-proxy"));
...
fn test_hybrid_entry_response_for_command_430_returns_none() {
    ...
    assert!(entry.response_for_command("ARTICLE", "<test@example.com>").is_none());
    
    // 430 stubs should not serve STAT either (article doesn't exist)
    assert!(entry.response_for_command("STAT", "<test@example.com>").is_none());
}
