...
fn test_article_entry_tier_default() {
    let entry = ArticleEntry::from_backend_response(b"220 0 <test@example.com>\r\n.\r\n");
    assert_eq!(entry.tier().get(), 0);
}

#[test]
fn test_article_entry_with_tier() {
    let entry = ArticleEntry::from_backend_response_with_tier(
...
fn test_article_entry_set_tier() {
    let mut entry = ArticleEntry::from_backend_response(b"220 0 <test@example.com>\r\n.\r\n");
    assert_eq!(entry.tier().get(), 0);
...
fn test_article_entry_is_expired_uses_tier() {
    let entry_tier_0 =
        ArticleEntry::from_backend_response_with_tier(b"220 test\r\n.\r\n", CacheTier::new(0));
    let entry_tier_1 =
        ArticleEntry::from_backend_response_with_tier(b"220 test\r\n.\r\n", CacheTier::new(1));
