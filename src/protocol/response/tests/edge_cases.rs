...
fn test_utf8_in_responses() {
    ...
    assert_eq!(parse_status_code(utf8_response), Some(StatusCode::new(200)));
    assert!(ResponseParser::is_greeting(utf8_response));

    // Auth response with UTF-8
    let utf8_auth = "281 认证成功\r\n".as_bytes();
    assert_eq!(parse_status_code(utf8_auth), Some(StatusCode::new(281)));
...
fn test_response_with_binary_data() {
    ...
    assert_eq!(parse_status_code(with_null), Some(StatusCode::new(200)));

    // Response with high bytes
    let with_high_bytes = b"200 \xFF\xFE\r\n";
    assert_eq!(
        parse_status_code(with_high_bytes),
