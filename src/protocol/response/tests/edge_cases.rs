...
fn test_utf8_in_responses() {
    ...
    assert!(matches!(
        NntpResponse::parse(utf8_response),
        NntpResponse::Greeting(_)
    ));

    // Auth response with UTF-8
    let utf8_auth = "281 认证成功\r\n".as_bytes();
    assert_eq!(StatusCode::parse(utf8_auth), Some(StatusCode::new(281)));
    assert_eq!(NntpResponse::parse(utf8_auth), NntpResponse::AuthSuccess);
}
...
fn test_response_with_binary_data() {
    ...
    assert_eq!(
        StatusCode::parse(with_high_bytes),
