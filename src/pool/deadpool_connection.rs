...
pub struct TcpManager {
    ...
    compression_support: Arc<Mutex<Option<CompressionSupport>>>,
...
impl TcpManager {
    ...
    fn is_ipv6_network_unreachable(socket_addr: SocketAddr, error: &ConnectionError) -> bool {
        socket_addr.is_ipv6()
            && matches!(
                error,
                ConnectionError::IoError(error) if error.raw_os_error() == Some(101)
            )
    }
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            compression_support: Arc::new(Mutex::new(None)),
...
impl TcpManager {
    ...
    async fn negotiate_compression(
        ...
        if self.compress == Some(true) {
            return self
                .probe_compression(stream, buffer)
                .await
        ...
        match *cached_support {
            ...
            None => {
                let support = self.probe_compression(stream, buffer).await?;
                *cached_support = Some(support);
                Ok(matches!(support, CompressionSupport::Supported))
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn ipv6_network_unreachable_matches_os_error_101_only_for_ipv6() {
        let ipv6_addr = SocketAddr::from(([0, 0, 0, 0, 0, 0, 0, 1], 563));
        let ipv4_addr = SocketAddr::from(([127, 0, 0, 1], 563));
        let error = ConnectionError::IoError(io::Error::from_raw_os_error(101));
