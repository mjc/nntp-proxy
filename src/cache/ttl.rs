...
///
/// assert_eq!(effective_ttl(1000, CacheTier::new(0)), 1000);  // 1x
/// assert_eq!(effective_ttl(1000, CacheTier::new(1)), 2000);  // 2x
/// assert_eq!(effective_ttl(1000, CacheTier::new(2)), 4000);  // 4x
/// assert_eq!(effective_ttl(1000, CacheTier::new(10)), 1_024_000); // 1024x
/// ```
#[inline]
#[must_use]
pub const fn effective_ttl(base_ttl_millis: u64, tier: CacheTier) -> u64 {
    let tier = tier.get();
...
/// # Examples
/// ```
/// use nntp_proxy::cache::ttl::{CacheTier, is_expired, now_millis};
///
/// let now = now_millis();
/// assert!(!is_expired(now, 1000, CacheTier::new(0))); // Just inserted, not expired
///
/// let old = now.saturating_sub(1500);
/// assert!(is_expired(old, 1000, CacheTier::new(0))); // 1.5s ago with 1s TTL = expired
/// assert!(!is_expired(old, 1000, CacheTier::new(1))); // 1.5s ago with 2s TTL = not expired
/// ```
#[inline]
#[must_use]
pub fn is_expired(inserted_at_millis: u64, base_ttl_millis: u64, tier: CacheTier) -> bool {
...
mod tests {
    ...
    fn effective_ttl_tier_0_is_base() {
        assert_eq!(effective_ttl(1000, CacheTier::new(0)), 1000);
        assert_eq!(effective_ttl(0, CacheTier::new(0)), 0);
        assert_eq!(effective_ttl(1, CacheTier::new(0)), 1);
    }

    #[test]
    fn effective_ttl_tier_1_is_2x() {
        assert_eq!(effective_ttl(1000, CacheTier::new(1)), 2000);
        assert_eq!(effective_ttl(500, CacheTier::new(1)), 1000);
    }

    #[test]
    fn effective_ttl_tier_2_is_4x() {
        assert_eq!(effective_ttl(1000, CacheTier::new(2)), 4000);
    }

    #[test]
    fn effective_ttl_tier_3_is_8x() {
        assert_eq!(effective_ttl(1000, CacheTier::new(3)), 8000);
    }

    #[test]
    fn effective_ttl_tier_7_is_128x() {
        assert_eq!(effective_ttl(1000, CacheTier::new(7)), 128_000);
    }

    #[test]
    fn effective_ttl_tier_10_is_1024x() {
        assert_eq!(effective_ttl(1000, CacheTier::new(10)), 1_024_000);
    }
    ...
    fn effective_ttl_saturates_on_overflow() {
        ...
        assert_eq!(effective_ttl(u64::MAX, CacheTier::new(1)), u64::MAX);
        assert_eq!(effective_ttl(u64::MAX, CacheTier::new(63)), u64::MAX);

        // Large value that would overflow without saturation
        // u64::MAX / 2 + 1 times 2 would overflow
        assert_eq!(effective_ttl(u64::MAX / 2 + 1, CacheTier::new(1)), u64::MAX);
    }
    ...
    fn is_expired_fresh_entry() {
        let now = now_millis();
        assert!(!is_expired(now, 1000, CacheTier::new(0))); // Just inserted, 1s TTL
        assert!(!is_expired(now, 100, CacheTier::new(0))); // Just inserted, 100ms TTL (1ms too tight for test timing)
    }

    #[test]
    fn is_expired_old_entry() {
        let old = now_millis().saturating_sub(2000);
        assert!(is_expired(old, 1000, CacheTier::new(0))); // 2s ago, 1s TTL = expired
        assert!(is_expired(old, 1999, CacheTier::new(0))); // 2s ago, 1.999s TTL = expired
    }
    ...
    fn is_expired_boundary() {
        ...
        assert!(!is_expired(inserted, 2000, CacheTier::new(0)));
    }
    ...
    fn is_expired_respects_tier() {
        ...
        assert!(is_expired(inserted, 1000, CacheTier::new(0))); // 1s TTL - expired
        assert!(!is_expired(inserted, 1000, CacheTier::new(1))); // 2s TTL - not expired
        assert!(!is_expired(inserted, 1000, CacheTier::new(2))); // 4s TTL - not expired
    }
    ...
    fn is_expired_high_tier_extends_ttl() {
        ...
        // With 1s base TTL
        assert!(is_expired(inserted, 1000, CacheTier::new(0))); // 1s TTL - expired
        assert!(is_expired(inserted, 1000, CacheTier::new(1))); // 2s TTL - expired
        assert!(is_expired(inserted, 1000, CacheTier::new(5))); // 32s TTL - expired
        assert!(is_expired(inserted, 1000, CacheTier::new(6))); // 64s TTL - expired
        assert!(!is_expired(inserted, 1000, CacheTier::new(7))); // 128s TTL - not expired
        assert!(!is_expired(inserted, 1000, CacheTier::new(10))); // 1024s TTL - not expired
    }
