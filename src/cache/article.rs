...
impl ArticleEntry {
    ...
    /// response.
    #[must_use]
    pub fn from_wire_response(buffer: Vec<u8>) -> Self {
...
mod tests {
    ...
    async fn test_weigher_large_articles() {
        ...
        let article = ArticleEntry::from_wire_response(response.as_bytes().to_vec());
    ...
    async fn test_weigher_small_stubs() {
        ...
        for i in 2..=200 {
            ...
            let article = ArticleEntry::from_wire_response(stub.as_bytes().to_vec());
            cache.insert(msgid, article).await;
        }
