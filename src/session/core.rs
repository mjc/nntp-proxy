//! Client session struct definition and builder pattern
//!
...
use super::{AuthState, ModeState, SessionMode};

/// Represents an active client session
pub struct ClientSession {
    pub(super) client_addr: ClientAddress,
    pub(super) buffer_pool: BufferPool,
    /// Unique identifier for this client
    pub(super) client_id: ClientId,
    /// Optional router for per-command routing mode
    pub(super) router: Option<Arc<BackendSelector>>,
...
mod tests {
    ...
    fn test_buffer_pool_cloning() {
        ...
        let _session1 = ClientSession::new(
            addr.into(),
            buffer_pool,
            test_auth_handler(),
            test_metrics(),
        );
        let _session2 = ClientSession::new(
            addr.into(),
            buffer_pool_clone,
            test_auth_handler(),
            test_metrics(),
    ...
    fn test_builder_basic_construction() {
        ...
        let session =
            ClientSession::builder(addr.into(), buffer_pool, auth_handler, test_metrics()).build();
    ...
    fn test_builder_with_metrics() {
        ...
        let session =
            ClientSession::builder(addr.into(), buffer_pool, test_auth_handler(), metrics).build();

        // Metrics is always present now, not optional
        let _ = session.metrics; // Just verify it exists
    }
    ...
    fn test_builder_with_cache() {
        ...
        let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8080);
        let buffer_pool = BufferPool::new(BufferSize::try_new(1024).unwrap(), 4);
        let cache = Arc::new(UnifiedCache::memory(100, Duration::from_secs(3600)));
    ...
    fn test_builder_method_chaining() {
        ...
        let session =
            ClientSession::builder(addr.into(), buffer_pool, test_auth_handler(), metrics)
                .with_router(router)
                .with_routing_mode(RoutingMode::Hybrid)
                .build();
